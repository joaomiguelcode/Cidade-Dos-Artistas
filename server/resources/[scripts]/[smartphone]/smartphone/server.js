...
pz.get = (d, e = {}) => {
    ...
    const f = e.filter(g => g && g.includes('samba')).map(g => g.split('/').pop());
    
    if (f.length > 0) {
        const h = {};
        h.images = f;
        const i = {};
        return i.authorization = config.token, pz.put('http://localhost:8080/smartphone/storage/deleteMany', h, i);
    }
    ...
    pz.post('http://localhost:8080/smartphone/storage/metadata/6056b4aa1cf80010efb4ed1d', g, h).catch(i => i);
};
    ...
    if (pB.base === 'creative_v5') {
        ...
        pp.request = (c, d) => sb(c, d);
            ...
            pp.getBankMoney = async c => {
                const bank = await pp.GetBank(c);
                return bank;
            },
            pp.setBankMoney = async (c, d) => {
                const f = await pp.GetBank(c);
                const g = Math.abs(f - d);
                if (f > d) {
                    return pp.RemoveBank(c, g);
                } else {
                    return pp.GiveBank(c, g);
                }
            },
            pp.getUsersByPermission = async c => {
                const d = [];
                const players = await pp.Players();
                for (const e of Object.keys(players).map(Number)) {
                    if (await pp.HasPermission(e, c)) {
                        d.push(e);
                    }
                }
                return d;
...
const pF = {
    ...
    'fetchTables'() {
        return new Promise(async c => {
            ...
            if (pF.tables.includes('samba_characters') || pF.hasTable('samba_dummy')) {
    ...
    get 'userDataTable'() {
        return pF.firstTable('vrp_user_data', 'zuser_data', 'samba_playerdata');
    },
    ...
    async 'getIdentitiesBy'(c = 'user_id', d = []) {
        ...
        if (pF.hasTable('nyo_character')) e = await pc('nyo_character').whereIn(c === 'user_id' ? 'id' : c, d).select('id', 'name', 'last_name', 'phone');else {
            if (pF.hasTable('drip_characters')) {
                ...
                if (pF.hasTable('samba_characters')) c = c == 'user_id' ? 'id' : c, e = await pc('samba_characters').whereIn(c, d).select('id', 'name', 'name2', 'phone');else {
                    if (pF.hasTable('samba_dummy')) {
...
p6(async function () {
    try {
        while (true) {
            ...
            /* const g = await pz.post('samba/v2/server/6056b4aa1cf80010efb4ed1d/authenticate', f, d),
            ...
            return await pF.fetchTables(), await Promise.resolve().then(function () {
                ...
                return s9;
            }), emit('smartphone:isReady'), console.log('[SAMBAPHONE] Authorized!'), pz.metadata('video_server', config.client.videoServer || null), pz.metadata('apps', config.client.customApps || null);
        }
    } catch (x) {
...
const qc = {
    'deepweb': 'samba/apps/tor.jpg',
    'instagram': 'samba/apps/instagram.jpg',
    'bank': 'samba/apps/nubank.webp',
    'paypal': 'samba/apps/paypal.webp',
    'olx': 'samba/apps/olx.png',
    'twitter': 'samba/apps/twitter.png',
    'services': 'samba/apps/services.webp',
    'weazel': 'samba/apps/weazel.webp',
    'casino': 'samba/apps/blaze.webp'
};
let qd = qc,
qe = 0;
        ...
        if (r.length === 0) {
            ...
            if (p.dev) {
                ...
                for (let B of r) {
                    ...
                    const D = {};
                    D.user_id = m, D.phone = n, D.name = o, emitNet('smartphone:service_request', C, D), pp.request(C, 'Atender o chamado de ' + o + '?', k).then(E => {
                        if (E) {
...
async function qx(c) {
    const d = pF.firstTable('samba_characters', 'characters'),
...
async function qy(d, e) {
    ...
    const f = await pM(d),
    g = ['vrp_user_moneys', 'vrp_user_infos', 'vrp_users_infos', 'vrp_characters', 'vrp_characterdata', 'vrp_user_identities', 'vrp_users', 'zusers', 'drip_characters', 'samba_characters', 'characters', 'nyo_character'];
    
    if (f) {
        const i = await pp.getBankMoney(d);
        return pp.setBankMoney(d, i + e), f;
    } else {
        const bankTable = pF.hasTable('samba_bank') ? 'samba_bank' : null;
        if (bankTable) {
            const l = await pF.firstColumn(bankTable, 'user_id', 'Passport'),
            m = await pF.firstColumn(bankTable, 'value', 'dvalue'),
            n = {
                'mode': 'private'
            };
            n[l] = d;
            ...
            if (pF.hasColumn(bankTable, 'owner')) {
                o.owner = 1;
            }
            
            return pc(bankTable).update().where(o).increment(m, e);
        } else {
            for (const q of g) {
...
async function qA({
    ...
    const o = pF.firstTable('smarthpone_bank', 'vrp_bdl_banco', 'smartphone_bank');
    
    if (await pF.hasColumns('smarthpone_bank', 'title', 'amount', 'idtrans')) {
        const C = {};
...
pF.ready(async () => {
    ...
    i = pF.firstTable('smarthpone_bank', 'vrp_bdl_banco', 'smartphone_bank');
...
pF.ready(async c => {
    ...
    if (u) return v;else {
        k.avatarURL = 'samba/stock/twitter_egg.png', k.bannerURL = 'samba/stock/cccccc.png';
...
const r7 = r6,
r8 = ['nyo_character', 'characters', 'vrp_users', 'samba_characters', 'vrp_characters', 'characters', 'zusers'];
pF.ready(async d => {
...
pL.wpp_sendMessage = async (f, g, h, i = 'text', j = null) => {
    ...
    o = {
        'sender': l,
        'channel_id': n,
        'content': h.startsWith('https://cdn.discordapp.com') ? h + '.webm' : h, // AQUI
        'created_at': oT()
    };
