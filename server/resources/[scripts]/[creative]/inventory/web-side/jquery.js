...
const updateDrag = () => {
	...
	$(".empty").droppable({
		...
		drop: function(event,ui){
			...
			if (origin === "invLeft" || origin.includes("ui-droppable")) {
				if (tInv === "invLeft"){
					$.post("http://inventory/updateSlot",JSON.stringify({
						item: itemData.key,
						slot: itemData.slot,
	...
	$.post("http://inventory/requestInventory",JSON.stringify({}),(data) => {
		...
		for (let x = 1; x <= 30; x++){
			...
			if (nameList2[x - 1] !== undefined){
				...
				const item = `<div class="item populated" data-item-key="${v["key"]}" data-id="${v["id"]}" data-amount="${v["amount"]}" data-peso="${v["peso"]}" data-slot="${slot}">
					<div class="top">
						<div class="itemWeight">${(v["peso"] * v["amount"]).toFixed(2)}</div>
						<div class="itemAmount">${formatarNumero(v["amount"])}x</div>
					</div>
					...
					<img src="http://181.215.236.140/inventory/${v["index"]}.png"></img>
					<div class="durability" style="--percent: ${actualPercent == 1 ? "100" : actualPercent}%; --color: ${colorPicker(actualPercent)}; opacity: ${(v["durability"] && v["durability"] > 0) ? 1 : 0} "></div>
					<div class="itemName">${v["name"]}</div>
				</div>`;
