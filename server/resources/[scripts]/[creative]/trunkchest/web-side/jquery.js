		...
		switch (event.data.action) {
			...
			case "updateWeight":
				...
				$(".header-weight").each(function (i) {
					$(this).find(".weight-texts p").html(
						`${(event.data[weights[i][0]]).toFixed(2)}<span>/${(event.data[weights[i][1]]).toFixed(2)}</span>`
					)
					$(this).find(".weight-icon svg g rect").css(
						"height",
						(event.data[weights[i][0]] / event.data[weights[i][1]]) * 100 + "%"
					);
				})
				break;
		}
...
const updateDrag = () => {
	...
	$(".items-slots li").droppable({
		...
		drop: function (event, ui) {
			...
			let amount = 0;
			const itemAmount = Number.parseInt(ui.draggable.data("amount"));

			if (shiftPressed) amount = itemAmount;
			else if (
				($(".amount").val() == "") |
				(Number.parseInt($(".amount").val()) <= 0)
			)
				amount = 1;
			else amount = Number.parseInt($(".amount").val());

			if (amount > itemAmount) amount = itemAmount;

			$(".populated, .bar, .empty, .use, .send, .deliver .destroy").off(
				"draggable droppable",
			);

			const clone1 = ui.draggable.clone();
			const slot2 = $(this).data("slot");

			if ($(this).children().length < 1) {
				if (amount == itemAmount) {
					const clone2 = $(this).clone();
					const slot1 = ui.draggable.data("slot");
					...
					if (origin.includes("ui-droppable")) {
						...
						ui.draggable.replaceWith(
							`<div class="item empty" data-slot="${slot1}"></div>`,
						);
					}
					...
					const newAmountOldItem = itemAmount - amount;
					const weight = Number.parseFloat(ui.draggable.data("peso"));
					const newWeightClone1 = (amount * weight).toFixed(2);
					const newWeightOldItem = (newAmountOldItem * weight).toFixed(2);
				...
				ui.draggable.data("item-key") == $(this).children().data("item-key")
			) {
				const newSlotAmount =
					amount + Number.parseInt($(this).children().data("amount"));
				const newSlotWeight = ui.draggable.data("peso") * newSlotAmount;
				...
				if (amount == itemAmount) {
					...
					const newMovedAmount = itemAmount - amount;
					const newMovedWeight =
						Number.parseFloat(ui.draggable.data("peso")) * newMovedAmount;
				...
				const clone2 = $(this).children().clone();
				const slot1 = ui.draggable.data("slot");
			...
			if (origin === "invLeft") {
				$.post("http://inventory/updateSlot",
					JSON.stringify({
						item: itemData.key,
						slot: itemData.slot,
						target: target,
	...
	$(".populated").droppable({
		...
		drop: function (event, ui) {
			...
			if (ui.draggable.data("item-key") == $(this).data("item-key")) {
				const newSlotAmount = amount + Number.parseInt($(this).data("amount"));
				const newSlotWeight = ui.draggable.data("peso") * newSlotAmount;
				...
				if (amount == itemAmount) {
					ui.draggable.replaceWith(
						`<div class="item empty" data-slot="${ui.draggable.data("slot")}"></div>`,
					);
				} else {
					const newMovedAmount = itemAmount - amount;
					const newMovedWeight =
			...
			if (origin === "invLeft" && tInv === "invLeft") {
				...
				$.post("http://trunkchest/updateChest",
					JSON.stringify({
						item: itemData.key,
						slot: itemData.slot,
						target: target,
	...
	$(".items-slots li").on("auxclick", doubleClick);

	$(".populated").on("auxclick", doubleClick);
...
const requestChest = () => {
	$.post("http://trunkchest/requestChest", JSON.stringify({}), (data) => {
		...
		$(".header-weight").each(function (i) {
			$(this).find(".weight-texts p").html(
				`${(data[weights[i][0]]).toFixed(2)}<span>/${(data[weights[i][1]]).toFixed(2)}</span>`
			)
			$(this).find(".weight-icon svg g rect").css(
				"height",
				(data[weights[i][0]] / data[weights[i][1]]) * 100 + "%"
			);
		})
		...
		for (let x = 1; x <= 5; x++) {
			...
			if (data["myInventory"][slot] !== undefined) {
				...
				const item = `<div class="item bar" title="" data-max="${v["max"]}" data-economy="${v["economy"]}" data-description="${v["desc"]}" data-amount="${v["amount"]}" data-peso="${v["peso"]}" data-item-key="${v["key"]}" data-name-key="${v["name"]}" data-slot="${slot}" data-idName="${v["idName"]}" data-idBlood="${v["idBlood"]}" data-idPremium="${v["idPremium"]}" data-garage="${v["Garage"]}" data-idVality="${v["idVality"]}" data-locate="${v["Locate"]}" data-visa="${v["Visa"]}" data-gems="${v["Gems"]}" data-idRolepass="${v["idRolepass"]}" data-Suitcase="${v["Suitcase"]}" data-Vehkey="${v["Vehkey"]}" data-Passport="${v["Passport"]}">
					...
					<div class="durability" style="--percent: ${actualPercent == 1 ? "100" : actualPercent}%; --color: ${colorPicker(actualPercent)}; opacity: ${data["myInventory"][slot]["durability"] > 0 ? 1 : 0} "></div>
		...
		for (let x = 1; x <= 50; x++) {
			...
			if (data["myChest"][x] !== undefined) {
				var v = data["myChest"][x];
				var maxDurability = 86400 * v["days"];
				var newDurability = (maxDurability - v["durability"]) / maxDurability;
				var actualPercent = newDurability * 100;
				...
				const item = `<div class="item populated" title="" data-max="${v["max"]}" data-economy="${v["economy"]}" data-description="${v["desc"]}" data-amount="${v["amount"]}" data-peso="${v["peso"]}" data-item-key="${v["key"]}" data-name-key="${v["name"]}" data-slot="${slot}" data-idName="${v["idName"]}" data-idBlood="${v["idBlood"]}" data-idPremium="${v["idPremium"]}" data-garage="${v["Garage"]}" data-idVality="${v["idVality"]}" data-locate="${v["Locate"]}" data-visa="${v["Visa"]}" data-gems="${v["Gems"]}" data-idRolepass="${v["idRolepass"]}" data-Suitcase="${v["Suitcase"]}" data-Vehkey="${v["Vehkey"]}" data-Passport="${v["Passport"]}">
					...
					<img src="http://181.215.236.140/inventory/${v["index"]}.png"></img>
					<div class="durability" style="--percent: ${actualPercent == 1 ? "100" : actualPercent}%; --color: ${colorPicker(actualPercent)}; opacity: ${data["myChest"][slot]["durability"] > 0 ? 1 : 0} "></div>
					<div class="itemName">${v["name"]}</div>
				</div>`;
