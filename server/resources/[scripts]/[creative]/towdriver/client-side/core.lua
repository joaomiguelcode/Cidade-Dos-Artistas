...
AddEventHandler("towdriver:invokeTow",function()
	...
	if (Model == `flatbed` or Model == `flatbed3` or Model == `guinchopmesp` or Model == `Eguinchoover` or Model == `energyguinchowcg`) and not IsPedInAnyVehicle(Ped) then
	...
	if NetworkDoesNetworkIdExist(veh01) and NetworkDoesNetworkIdExist(veh02) then
		...
		if DoesEntityExist(vehicle) and DoesEntityExist(vehTowed) then
			...
			elseif mode == "out" then
				...
				SetEntityCoords(vehTowed,vehCoords["x"],vehCoords["y"],vehCoords["z"] +0.3,false,false,false,false)
