...
const functionSearch = (passaporte) => {
	if ( passaporte !== "" ) {
		$.post("http://police/searchUser", JSON.stringify({ passaporte: passaporte }), (data) => {
			if ( data["result"][0] === true ) {
				$("#content").html(`
					<div id="titleContent">${data["result"][1]}</div>
					<div id="pageLeftSearch">
						<div class="searchBox">
							<b>Passaporte:</b> ${data["result"][8]}<br>
...
const displayGoogleDoc = (doc_id) => {
	$("#content").html(`<iframe src="https://docs.google.com/" style="width: 100%;height: 95%;border: none"></iframe>`);
};
	...
	$("#content").html(`
		...
		<div id="pageLeft">
			...
			<div style="display: inline-block">
				...
				<input class="inputPrison" id="prenderAssociacao" type="text" value="" placeholder="Associação (Ex: 1, 2, 3)."/>
			</div>
