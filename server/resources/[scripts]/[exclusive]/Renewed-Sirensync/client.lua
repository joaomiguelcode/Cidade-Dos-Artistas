...
lib.onCache('seat', function(seat)
    ...
    SetTimeout(0, function()
        ...
        if not Entity(cache.vehicle).state.stateEnsured then
            TriggerServerEvent('Renewed-Sirensync:server:SyncState', VehToNet(cache.vehicle), isVehAllowed())
        end
...
local function stateBagWrapper(keyFilter, cb)
    return AddStateBagChangeHandler(keyFilter, '', function(bagName, _, value, _, replicated)
        ...
        local success, result = pcall(function()
            ...
            end, 'Timeout while waiting for entity to exist', 3000) -- reduzido
        end)

        loaded = success and result
...
-- Flash Alerts --
local alertsVehicles = {}

local flashAlert = lib.addKeybind({
    name = 'flashAlert',
    description = 'Press this button to use the flash alert',
    defaultKey = Config.Controls.flashAlert,
    onPressed = function()
        local state = Entity(cache.vehicle).state
        ...
        local curMode = state.flashAlert
        state:set('flashAlert', not curMode, true)
    end
})

stateBagWrapper('flashAlert', function(veh, stats)
    if stats then
        CreateThread(function()
            while Entity(veh).state.flashAlert do
                for veh, stats in pairs(alertsVehicles) do
                    alertsVehicles[veh] = not stats
                    SetVehicleIndicatorLights(veh,0,alertsVehicles[veh])
                    SetVehicleIndicatorLights(veh,1,alertsVehicles[veh])
                end
                Wait(500)
            end
        end) 
    else
        alertsVehicles[veh] = nil
        SetVehicleIndicatorLights(veh,0,false)
        SetVehicleIndicatorLights(veh,1,false)
        return
    end
    alertsVehicles[veh] = stats
end)