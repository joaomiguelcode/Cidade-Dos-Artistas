...
window.addEventListener('message', function(event) {
    ...
    if (ed.action === "menu") {
		...
		ed.categories.forEach(function(categoryData, index) {
            let url = de(getMenuUrl(categoryData.image));
            ...
            <div class="MDLSBCategoryDiv" id="MDLSBCategoryDiv-${categoryData.name}" onclick="clFunc('chooseCategory', '${categoryData.name}', '${categoryData.number}')">
                ...
                <p id="MDLSBCategoryDivTexts2-${categoryData.name}">${categoryData.label} </p>
            </div>`;
        ...
        if (currentCategory === "propemotes") {
            if (ed.state === true) {
                document.querySelectorAll('.geniunanim').forEach(animDiv => {
        ...
        if (name2 === "favorites") {
            ...
            favoriteAnimations.forEach(function(favData, index) {
                ...
                $(`#mainDivRightSideBottomTopDiv-${favData.id}`).hover(function() {
                    $(`#MDRSBTDTopDivCommand-${favData.id}`).html(`<h4>/a ${favData.name}</h4>`);
                }, function() {
...
function updateContent(emotes) {
    ...
    emotes.forEach((item, index) => {
        ...
        if (existingAnim) {
            ...
            let src1 = de(getUrl(category, item.imgId));
            let src2 = de(getUrlImg(category, item.imgId));
            if (category === "placedemotes" || category == "syncedemotes") {src2 = "files/unknown.png"};
            const divItem = document.createElement('div');
            $(divItem).data("animData", item);
            $(divItem).hover(function() {
                $(`#MDRSBTDTopDivCommand-${item.id}`).html(`<h4>/a ${item.name}</h4>`);
            }, function() {
                $(`#MDRSBTDTopDivCommand-${item.id}`).html(`<h4>${name}</h4>`);
            });
            if (layout == 0) {
                divItem.innerHTML = `
                <div id="lay0" class="geniunanim flex flex-col justify-end items-start px-[0.2vw] py-[1.4vh] w-[7.2917vw] h-[12.963vh] shrink-0 border [background:rgba(255,255,255,0.03)] rounded-[.2604vw] border-solid border-[rgba(255,255,255,0.05)]"  onclick="clFunc('playAnim', '${item.animId}', '${item.category}')">
                ...
                <div class="flex justify-center items-center w-full h-[70%]" id="geniun-${item.id}">
                    <img id="geniun-img-${item.id}" class=""h-full" loading="lazy" src="${src2}"/>
                </div>
                <div class="w-full h-[25%]">
                        <div class="w-full h-[20%]"></div>
                        <div class="flex justify-center items-center w-full h-[30%] text-[#FFF] [font-family:'DM_Sans'] text-[.7292vw] textttb">${label}</div>
                ...
                divItem.innerHTML = `
                <div id="lay1" class="geniunanim flex w-[15.2083vw] h-[5.5556vh] shrink-0 border [background:rgba(255,255,255,0.03)] rounded-[.2604vw] border-solid border-[rgba(255,255,255,0.05)]"  onclick="clFunc('playAnim', '${item.animId}', '${item.category}')">
                <img onclick="clFunc('addAnimToFavorites', '${item.id}')">
                            ...
                            <div class="flex w-[80%] h-full">
                                <div class="w-[50%] h-full">
                                    <div class="w-full h-[20%]"></div>
                                    <div class="w-full h-[30%] text-[#FFF] [font-family:'DM_Sans'] text-[.7292vw] textttb">${label}</div>
                                    <div class="w-full h-[30%] text-[#AAA] [font-family:'DM_Sans'] text-[.6771vw] textttb"> ${name}</div>
    ...
    emotes.forEach((item, index) => {
        let category = item.category;
        //if (category === "general" || category === "dances") {
            let src1 = de(getUrl(category, item.imgId));
            let src2 = de(getUrlImg(category, item.imgId));
            if (category === "placedemotes" || category == "syncedemotes") {src2 = "files/unknown.png"};
            let img = document.getElementById(`geniun-img-${item.id}`);
            if (img) {
...
function addAnimToQuick(data, id) {
    ...
    let src = de(getUrl(category, data.imgId));
    if (category === "placedemotes" || category == "syncedemotes") {src = "files/unknown.png"};
