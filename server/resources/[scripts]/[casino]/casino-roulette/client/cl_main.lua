...
createRulettAsztal = function(index, data)
    ...
    local tableModel = GetHashKey('vw_prop_casino_roulette_01')
    RequestModel(tableModel)
    local tableTimeout = GetGameTimer() + 5000
    while not HasModelLoaded(tableModel) do
        Wait(1)
        if GetGameTimer() > tableTimeout then
            TriggerEvent("Notify","aviso","Falha ao carregar a mesa de roleta. Por favor, tente novamente.",5000)
            return nil
        end
    end

    self.tableObject = CreateObject(tableModel, data.position, false)
    SetEntityHeading(self.tableObject, data.rot)

    local dealerModel = GetHashKey('S_F_Y_Casino_01')
    RequestModel(dealerModel)
    local dealerTimeout = GetGameTimer() + 5000
    while not HasModelLoaded(dealerModel) do
        Wait(1)
        if GetGameTimer() > dealerTimeout then
            TriggerEvent("Notify","aviso","Falha ao carregar o dealer da roleta. Por favor, tente novamente.",5000)
            if self.tableObject then DeleteObject(self.tableObject) end
            return nil
        end
    end

    local pedOffset = GetObjectOffsetFromCoords(data.position.x, data.position.y, data.position.z, data.rot, 0.0, 0.7, 1.0)
    self.ped = CreatePed(2, dealerModel, pedOffset, data.rot + 180.0, false, true)
                ...
                while selectedRulett ~= nil do
                    ...
                    if Config.allowCustomBet then
                        if IsDisabledControlJustPressed(0, 22) then --Custom Bet [space]
                            local tmpInput = getGenericTextInput('Quantas fichas você gostaria de apostar?')
                            if tonumber(tmpInput) then
                                tmpInput = tonumber(tmpInput)
                                if tmpInput > 0 then
                                    changeBetAmount(tmpInput)
                                    TriggerEvent("Notify","aviso",'Aposta Personalizada: ' ..currentBetAmount.. ' fichas.',5000)
                                end
                            end
                        end
                    else
                                        ...
                                        if currentBetAmount > 0 then
                                            if Config.RulettTables[selectedRulett] ~= nil then
                                                if currentBetAmount >= Config.RulettTables[selectedRulett].minBet and currentBetAmount <= Config.RulettTables[selectedRulett].maxBet then
                                                    ...
                                                    TriggerEvent("Notify","aviso","Sua aposta está muito baixa ou muito alta para esta mesa.",5000)
                                                end
                                            end
                                        else
                                            TriggerEvent("Notify","aviso","A aposta precisa ser aumentada.",5000)
                                        end
                                    end
                                end
                            end
...
function casinoNuiUpdateGame(rulettIndex, ido, statusz)
    local result = lib.callback.await('roulette:server:ChipsAmount')
    retval = result
    if selectedRulett == rulettIndex then
        if not statusz then
            exports['casinoUi']:DrawCasinoUi('show', "Cassino CDA</p>Tempo Restante: "..ido.."</p>Aposta Atual: "..currentBetAmount.."</p>Fichas Disponíveis: "..math.floor(retval))
            if Config.allowCustomBet then
                lib.showTextUI("Place Bet: LEFT CLICK | Adjust Bet: | >↑/↓< | Exit: | ← | Custom Amount:|  SPACEBAR")
            else
                lib.showTextUI("Place Bet: | LEFT CLICK | Adjust Bet: | ↑/↓ | Exit: | ←")
            end
        else
            lib.showTextUI("The game is starting..")
            hideUi()
        end
    end
end
