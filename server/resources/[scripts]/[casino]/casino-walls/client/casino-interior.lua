-- casino-walls: client/casino-interior.lua
-- Handles animated walls, car display, and ambient audio in the casino
...
-- Helper: Request a model with timeout
local function safeRequestModel(model, timeoutMs)
    ...
    while not HasModelLoaded(model) do
        ...
        if GetGameTimer() - start > timeoutMs then
            print('[casino-walls] Failed to load model: '..tostring(model))
            return false
        end
    end
    return true
...
-- Start threads for animated video wall
...
AddEventHandler("chCasinoWall:enteredCasino", function()
    ...
    SetTimeout(1000, function()
        TriggerEvent('casino:unhidePeds')
    end)
end)
...
RegisterNetEvent('casino-walls:client:syncWallTheme')
AddEventHandler('casino-walls:client:syncWallTheme', function(theme)
    Config.AnimatedWallNormal = theme
    if inCasino and not showBigWin then
        setVideoWallTvChannel()
    end
end)
...
function enterCasino()
    ...
    TriggerServerEvent('chCasinoWall:server:enteredCasino')
    print("Entered Casino area")
end

function exitCasino()
    TriggerEvent("chCasinoWall:exitedCasino")
    TriggerServerEvent('chCasinoWall:server:exitedCasino')
...
CreateThread(function()
    ...
    CasinoZone:onPlayerInOut(function(isInside)
        if isInside then
            enterCasino()
        else
            exitCasino()
...
function drawCarForWins()
    ...
    if not safeRequestModel(Config.VehicleOnDisplay, 5000) then
        print('[casino-walls] Failed to load car model for display!')
        return
    end
    SetModelAsNoLongerNeeded(Config.VehicleOnDisplay)
    spinningCar = CreateVehicle(Config.VehicleOnDisplay, 2685.29, 3498.86, 55.52, 0.0, 0, 0)
    Wait(0)
...
function playSomeBackgroundAudioBaby()
    CreateThread(function()
        local function audioBanks()
            while not RequestScriptAudioBank("DLC_VINEWOOD/CASINO_GENERAL", false, -1) do Wait(0) end
            while not RequestScriptAudioBank("DLC_VINEWOOD/CASINO_SLOT_MACHINES_01", false, -1) do Wait(0) end
            while not RequestScriptAudioBank("DLC_VINEWOOD/CASINO_SLOT_MACHINES_02", false, -1) do Wait(0) end
            while not RequestScriptAudioBank("DLC_VINEWOOD/CASINO_SLOT_MACHINES_03", false, -1) do Wait(0) end
        end
        audioBanks()
        while inCasino do
...
-- Fix: Enable gunfire sounds inside the casino by stopping the casino audio scene when shooting
CreateThread(function()
    local restoreTimer = 0
    while true do
        Wait(0)
        if inCasino and IsPedShooting(PlayerPedId()) then
            StopAudioScene("DLC_VW_Casino_General")
            restoreTimer = GetGameTimer() + 3000 -- 3 seconds from now
        end
        -- Restore ambience after 3 seconds if player is still in the casino
        if inCasino and restoreTimer > 0 and GetGameTimer() > restoreTimer then
            StartAudioScene("DLC_VW_Casino_General")
            restoreTimer = 0
        end
        if not inCasino then
