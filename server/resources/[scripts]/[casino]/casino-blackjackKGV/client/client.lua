...
RegisterNetEvent("BLACKJACK:RequestBets", function(index)
	...
	local result = lib.callback.await('BLACKJACKKGV:server:blackChipsAmount')
	retval = result
	CreateThread(function()
		scrollerIndex = index
		lib.showTextUI("Place Bet: | ↵ | Adjust Bet: | ↑/↓ | Exit: | ←")
		while true do
			Wait(0)
			exports['casinoUi']:DrawCasinoUi('show', "Cassino CDA | Blackjack </p>Time Left: 0:"..timeLeft.."</p>Current Bet: "..bet.." </p>Availble chips: "..math.floor(retval))   
			...
			if IsControlJustPressed(1, 201) then -- ENTER / A
				...
				if canBet then
					...
					TriggerEvent("Notify","aviso","Você não tem fichas no cassino...",5000)
				end
			end
		end
	end)
...
RegisterNetEvent("ry::client:blackjackMenu", function(args)
	...
	elseif args == 3 then
		...
		if canBet then
			...
			TriggerEvent("Notify","aviso","Você não tem fichas suficientes no cassino para dobrar a aposta.",5000)
			exports['casinoUi']:DrawCasinoUi('show', "Cassino CDA | Blackjack </p>Dealer: "..dealerValue[g_seat].."</p>Mão: "..handValue(hand))  
		...
		if canBet then
			...
			TriggerEvent("Notify","aviso","Você não tem fichas suficientes no cassino para dividir.",5000)
			exports['casinoUi']:DrawCasinoUi('show', "Cassino CDA | Blackjack </p>Dealer: "..dealerValue[g_seat].."</p>Mão: "..handValue(hand))  
			TriggerEvent("casino:context:hit&stand")
		end
	end 
...
RegisterNetEvent("BLACKJACK:RequestMove", function()
	...
	elseif CanSplitHand(hand) == true then
		...
		exports['casinoUi']:DrawCasinoUi('show', "Cassino CDA | Blackjack </p>Dealer: "..dealerValue[g_seat].."</p>Mão: "..handValue(hand).."</p>[Dividir Mão: "..handValue(splitHand).."]") 
				...
				if #(GetEntityCoords(PlayerPedId()) - vector3(cord.x, cord.y, cord.z)) < 3.0 then
					...
					if GetEntityCoords(tableObj) ~= vector3(0.0, 0.0, 0.0) then
						...
						if #(pCoords - coords) < 1.7 and not IsSeatOccupied(coords, 0.5) and canSit then
							...
							if highStakes then
								text = "Cassino CDA | Blackjack (High-Limit) | Pressione E para sentar"  
							else
								text = "Cassino CDA | Blackjack  | Pressione E para sentar" 
							end
...
-- Helper: Load a model with timeout and error handling
