...
AddEventHandler(
    ...
    function(tableId, Cards)
        if not SharedPokers[tableId] then
            TriggerEvent("Notify","aviso","Falha ao iniciar a mesa de poker. Por favor, tente novamente.",5000)
            return
        end
...
AddEventHandler(
    ...
    function(amount)
        if not amount or type(amount) ~= 'number' or amount < 0 then
            TriggerEvent("Notify","aviso","Falha ao atualizar fichas: valor inválido.",5000)
            return
        end
...
AquiverPoker = function(index, data)
    ...
    self.sitDown = function(chairId, chairCoords, chairRotation)
        ...
        if not IsEntityDead(PlayerPedId()) then
            local canSit = lib.callback.await('aquiverPoker:sitDown', false, self.index, chairId)
            if canSit then
                activeChairData = {
                    chairId = chairId,
                ...
                TriggerEvent("Notify","aviso","Este assento está ocupado.",5000)
            end
        end
    end
                    ...
                    while clientTimer ~= nil do
                        ...
                        if clientTimer ~= nil then
                            ...
                            if clientTimer < 1 then
                                clientTimer = nil
                                TriggerEvent("Notify","aviso","Você não respondeu à solicitação do dealer a tempo, sua mão foi desistida.",5000)
                ...
                if GetPlayerServerId(PlayerId()) == targetSrc and self.index == activePokerTable then
                    ...
                    Citizen.CreateThread(
                        function()
                            ...
                            lib.showTextUI("Play Hand: | E | Fold Hand: | ←")
                                    ...
                                    Citizen.CreateThread(
                                        function()
                                            while watchingCards do
                                                Citizen.Wait(0)
                                                exports['casinoUi']:DrawCasinoUi('show', "<strong>Cassino CDA</strong></p>Mão do Jogador: "..form)
                ...
                function()
                    while activePokerTable do
                        ...
                        if buttonScaleform ~= nil then
                            ...
                            exports['casinoUi']:DrawCasinoUi('show', "<strong>Cassino CDA</strong></p>Aposta Atual: "..currentBetInput.."</p>Fichas Disponíveis: "..PlayerOwnedChips)
                        end
                            ...
                            if self.TimeLeft == nil or self.TimeLeft > 0 then
                                ...
                                if IsDisabledControlJustPressed(0, 176) then
                                    if currentBetInput > 0 then
                                        ...
                                        TriggerEvent("Notify","aviso","Você não definiu um valor de aposta.",5000)
                                    end
                                end
                            ...
                            Citizen.CreateThread(
                                function()
                                    while DoesCamExist(mainCamera) do
                                        ...
                                        exports['casinoUi']:DrawCasinoUi('show', "<strong>Cassino CDA</strong></p>Mão do Dealer: "..form)
                                ...
                                for chairBone, chairId in pairs(Config.PokerChairs) do
                                    ...
                                    if chaircoords then
                                        if #(playerpos - chaircoords) < 1.5 then
                                            wait = 5
                                            inZone  = true
                                            text = "Cassino CDA | Poker | Pressione E para sentar"
