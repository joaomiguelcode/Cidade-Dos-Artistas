...
function setGender(gender)    
    ...
    if gender == "female" or gender == "feminino" or gender == "F" then
        model = "mp_f_freemode_01"
    elseif gender == "male" or gender == "masculino" or gender == "M" or not gender or gender == "" then
        model = "mp_m_freemode_01"
    elseif type(gender) == "string" or type(gender) == "number" then
        model = gender
    end
    ...
    local weapons = (vRP.getWeapons and vRP.getWeapons()) or {}
    ...
    if vRP.giveWeapons then vRP.giveWeapons(weapons,true) end
    resetCloths()
    if gender == "female" or gender == "feminino" or gender == "F" then
...
function loadingPlayer(stats)
    TriggerEvent('nation_hud:updateHud', stats)
    TriggerEvent("hud:Active", stats)
    local ped = PlayerPedId()
    SetEntityInvincible(ped,not stats)
    SetEntityVisible(ped,stats)
    FreezeEntityPosition(ped,not stats)
    LocalPlayer["state"]["Invincible"] = not stats
...
RegisterNetEvent("nation_creator:setupChars")
AddEventHandler("nation_creator:setupChars", openCharacterSelect)

RegisterNetEvent("spawn:setupChars")
AddEventHandler("spawn:setupChars", openCharacterSelect)
...
function selectCharacter(info)
    ...
    if inMultiChar then
        local ped = PlayerPedId()
        FreezeEntityPosition(ped, true)
        SetEntityVisible(ped, info ~= nil)
        local x,y,z = table.unpack(creatorCoords)
        SetEntityCoords(ped, x, y, z - 0.9, false, false, false, true)
        SetEntityHeading(ped, creatorHeading)
    end
end
...
function setClothing(data)
    ...
    for comp, i in pairs(components) do
        if data[comp] and data[comp].item then
            SetPedComponentVariation(ped,i,data[comp].item,data[comp].texture or 0,2)
        end
    end
    for prop, i in pairs(props) do
        if data[prop] and data[prop].item and data[prop].item > 0 then
            SetPedPropIndex(ped,i,data[prop].item,data[prop].texture or 0,2)
        else
            ClearPedProp(ped,i)
        end
    end
