...
setPlayerChar = function(char, forceGender)
    if not char or type(char) ~= "table" then return end
    player = char
    local ped = PlayerPedId()
    local charGender = char.gender
    if not charGender or charGender == "" then
        charGender = getGender(ped)
    end
    if charGender ~= getGender(ped) or forceGender then
...
setClothes = function(custom)
    ...
    for k,v in pairs(custom or {}) do
        if type(v) == "table" and v[1] then
            ...
            if isprop and index then
                if v[1] < 0 then
                    ClearPedProp(ped,index)
                else
                    SetPedPropIndex(ped,index,v[1],v[2] or 0,v[3] or 2)
                end
            elseif index and index >= 0 and index < 12 then
                SetPedComponentVariation(ped,index,v[1],v[2] or 0,v[3] or 2)
            end
        end
    end
end
...
local changeChar = function(data)
    if not inMenu or (not data or not data.key) or not scriptLoaded then return end
...
local finishCreation = function(data, cb)
    if not inMenu or not scriptLoaded then return cb(false, "ERROR") end
    local success = func.saveChar(name, lastName, age, getPlayerChar(PlayerPedId()))
    if success then
        cb(true)
...
startCreator = function()
    if not scriptLoaded or inMenu or inMultiChar then return end
    inMenu = true
    initCreator()
...
RegisterNetEvent("nation_creator:startCreator")
AddEventHandler("nation_creator:startCreator", function()
    startCreator()
end)

RegisterNetEvent("nation_creator:open")
AddEventHandler("nation_creator:open", function()
    startCreator()
end)
...
RegisterNetEvent("nation_creator:setPlayerChar")
AddEventHandler("nation_creator:setPlayerChar", function(char)
    if char then
        if type(char) == "string" then
            char = json.decode(char)
        end
        if char and type(char) == "table" then
            setPlayerChar(char, true)
        end
    end
end)
...
RegisterNUICallback("loaded", function(data, cb)
    if not scriptLoaded or loaded then return end
    loaded = true
end)
...
function toggleMultiChar()
    if inMenu or not scriptLoaded then return end
...
local multiCharButtons = {
    selectChar = function(data, cb)
        if not inMultiChar then 
            if cb then cb(false) end
            return 
        end
        local index = data.char+1
        local chars = myChars and myChars.chars
        if chars and chars[index] then
...
function toggleLogin(stats)
    if inMenu or inMultiChar or not scriptLoaded then return end
    if stats ~= nil then 
        inLoginMenu = stats
    else
...
RegisterNUICallback("changeSpawnCam", function(data, cb)
    if not inLoginMenu or not scriptLoaded then return end
