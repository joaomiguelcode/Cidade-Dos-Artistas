...
skinshops = {
    ...
    [18] = {
        clothes = getClothes, permission = nil, coords = vec3(-2055.28,-511.1,12.82)   
    },
    [19] = {
        clothes = getClothes, permission = nil, coords = vec3(-2064.38,-522.31,12.82)   
    },
    [20] = {
        clothes = getClothes, permission = nil, coords = vec3(-2058.84,-527.22,12.82)     
    },
    [21] = {
        clothes = getClothes, permission = nil, coords = vec3(-2058.3,-517.99,12.82)    
    },
...
mainThread = function()
    -- Adiciona a lógica de encontrar as lojas mais próximas
    ...
    -- Cria a thread que ficará buscando lojas próximas
    CreateThread(getNearestSkinshops)

    -- Loop principal para desenhar texto 3D e marcador
    while true do
        local idle = 500
        local ped = PlayerPedId()
        ...
        if not inMenu then
            for skinShopId, v in pairs(nearestSkinshops) do
                if v and v.coords and GetEntityHealth(ped) > 101 then
                    idle = 5  -- Reduz o tempo de espera para atualização frequente

                    local coords = v.coords
                    -- Desenha o texto 3D próximo à loja
                    DrawText3D(coords.x, coords.y, coords.z, "~w~[E]~w~ PARA ACESSAR")

                    DrawMarker(27,coords.x, coords.y , coords.z-0.9,0,0,0,0,0.0,0.0,0.5, 0.5, 0.5, 71, 71, 71, 100,0,0,0,1) 

                    local distance = #(myCoords - coords)
                    -- Se o jogador pressionar a tecla [E] e estiver perto da loja, abre o menu
