...
-- =========================================================================
-- REMOTE / SERVER-SIDE MENU BRIDGE
-- =========================================================================

local ServerMenuPool = nil
local CurrentServerMenu = nil
local CurrentServerMenuId = nil

RegisterNetEvent("NativeUI:Client:ShowNotification", function(message)
    SetNotificationTextEntry("STRING")
    AddTextComponentString(message or "")
    DrawNotification(false, false)
end)

RegisterNetEvent("NativeUI:Client:CloseMenu", function(menuId)
    if CurrentServerMenu then
        if not menuId or menuId == CurrentServerMenuId then
            CurrentServerMenu:Visible(false)
            CurrentServerMenu = nil
            CurrentServerMenuId = nil
        end
    end
end)

RegisterNetEvent("NativeUI:Client:OpenMenu", function(menuData)
    if not menuData or type(menuData) ~= "table" then return end

    if ServerMenuPool and CurrentServerMenu and CurrentServerMenu:Visible() then
        CurrentServerMenu:Visible(false)
    end

    ServerMenuPool = NativeUI.CreatePool()
    local title = menuData.title or "Menu"
    local subtitle = menuData.subtitle or ""
    local menuId = menuData.menuId or tostring(GetGameTimer())
    CurrentServerMenuId = menuId

    local menu = NativeUI.CreateMenu(title, subtitle)
    ServerMenuPool:Add(menu)
    CurrentServerMenu = menu

    if menuData.items and type(menuData.items) == "table" then
        for index, itemConfig in ipairs(menuData.items) do
            local itemType = itemConfig.type or "item"
            local itemId = itemConfig.id or index
            local itemObj = nil

            if itemType == "checkbox" then
                itemObj = NativeUI.CreateCheckboxItem(itemConfig.text or "Opção", itemConfig.checked or false, itemConfig.description or "")
                menu:AddItem(itemObj)
                itemObj.CheckboxEvent = function(sender, item, checked)
                    TriggerServerEvent("NativeUI:Server:OnCheckboxChange", menuId, itemId, checked)
                end
            elseif itemType == "list" then
                local listItems = itemConfig.items or {}
                local listIndex = itemConfig.index or 1
                itemObj = NativeUI.CreateListItem(itemConfig.text or "Lista", listItems, listIndex, itemConfig.description or "")
                menu:AddItem(itemObj)
                itemObj.OnListChanged = function(sender, item, newIndex)
                    TriggerServerEvent("NativeUI:Server:OnListChange", menuId, itemId, newIndex, listItems[newIndex])
                end
            elseif itemType == "slider" then
                local sliderItems = {}
                local maxVal = itemConfig.max or 10
                for s = 1, maxVal do sliderItems[s] = s end
                itemObj = NativeUI.CreateSliderItem(itemConfig.text or "Slider", sliderItems, itemConfig.index or 1, itemConfig.description or "")
                menu:AddItem(itemObj)
                itemObj.OnSliderChanged = function(sender, item, newIndex)
                    TriggerServerEvent("NativeUI:Server:OnSliderChange", menuId, itemId, newIndex)
                end
            elseif itemType == "coloured" then
                local mc = itemConfig.mainColour or { 0, 150, 255 }
                local hc = itemConfig.highlightColour or { 255, 255, 255 }
                itemObj = NativeUI.CreateColouredItem(itemConfig.text or "Opção", itemConfig.description or "", mc, hc)
                menu:AddItem(itemObj)
            else
                itemObj = NativeUI.CreateItem(itemConfig.text or "Opção", itemConfig.description or "")
                menu:AddItem(itemObj)
            end

            -- Click event
            if itemObj then
                itemObj.Activated = function(sender, item)
                    TriggerServerEvent("NativeUI:Server:OnItemSelected", menuId, itemId, index, itemConfig.extraData)
                end
            end
        end
    end

    menu.OnMenuClosed = function(sender)
        TriggerServerEvent("NativeUI:Server:OnMenuClosed", menuId)
        CurrentServerMenu = nil
        CurrentServerMenuId = nil
    end

    ServerMenuPool:RefreshIndex()
    menu:Visible(true)

    CreateThread(function()
        while CurrentServerMenu and CurrentServerMenu:Visible() do
            Wait(0)
            if ServerMenuPool then
                ServerMenuPool:ProcessMenus()
            end
        end
    end)
end)
